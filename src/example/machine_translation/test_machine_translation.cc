...
TEST(NMT, TestTrainCPU) {
  ...
  auto beam_search_decoder = [&](std::vector<VariableHandle> contexts,
    ...
    for (auto context : contexts) {
      ...
      // Initial candidate is {<s> (start token), 0.0f (beam search score)}
      ...
      for (int i = 1; i <= max_len; ++i) {
        ...
        // finished if there is remaining beam search quota
  ...
  for (int i = 0; i < test_steps; ++i) {
    ...
    for (int j = 0; j < batch_size; ++j) {
      LOG(INFO) << "Beam search decoder result for the #" << j
                << " input in step " << i << " :";
      for (int k = 0; k < beam_size; ++k) {
        LOG(INFO) << "#" << k << " beam search candidate:";
        LOG(INFO) << "Score = " << result[j][k].second;
        std::string seq("[");
        for (auto num : result[j][k].first) {
          seq.append(std::to_string(num) + ",");
        }
        seq.pop_back();
        seq.push_back(']');
        LOG(INFO) << "Word sequence = " << seq;
      }
    }
  }
